...
```
edit_tasks({tasks: [
  {id: "task-id", type: "advance"},
  {id: "task-id2", type: "advance"},
  etc...
]})
```

Once the tasks are in reviewing, spawn 1-4 "task-reviewer" subagents to review the tasks for completion.
...
Next, claim new tasks and repeat until all tasks are complete.
